...
def dividirEBalancearPorClasse(n_classes_balanceamento, test_size):
    pasta_ler = 'E:\\datasets\\imagens\\6classes\\'
...
def operacaoAugmentation_retornar(operacao, img, tamanho=(90,90)):
    if img is None:
        print("⚠️ Imagem None encontrada, usando placeholder")
        nova_img = np.zeros((tamanho[0], tamanho[1], 3), dtype=np.uint8)
        return nova_img

    # Se for grayscale, garante 3 canais
    if len(img.shape) == 2:
        print("🔹 Convertendo grayscale para BGR")
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    elif img.shape[2] != 3:
        print(f"🔹 Ajustando número de canais de {img.shape[2]} para 3")
        img = img[:, :, :3]

    # Redimensiona para tamanho fixo
    if img.shape[:2] != tamanho:
        print(f"🔹 Redimensionando de {img.shape[:2]} para {tamanho}")
    ...
    # Espelhar
        ...
        nova_img = cv2.flip(cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE), 1)
    elif operacao == 6:
        nova_img = cv2.flip(cv2.rotate(img, cv2.ROTATE_180), 1)
    elif operacao == 7:
        nova_img = cv2.flip(cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE), 1)
    # Ruído e denoise
    elif operacao == 8:
        sigma = 0.05
        nova_img = np.clip(random_noise(img, var=sigma**2)*255,0,255)
    elif operacao == 9:
        sigma = 0.005
        noisy = random_noise(img, var=sigma**2)
        nova_img = np.clip(denoise_tv_chambolle(noisy, weight=0.05, channel_axis=-1)*255,0,255)
    elif operacao == 10:
        sigma = 0.005
        noisy = random_noise(img, var=sigma**2)
        nova_img = np.clip(denoise_bilateral(noisy, sigma_color=0.01, sigma_spatial=5, channel_axis=-1)*255,0,255)
    else:
        nova_img = img

    # Garantir uint8 e shape (H,W,3)
    nova_img = nova_img.astype(np.uint8)
    if len(nova_img.shape) == 2:
        print("🔹 Convertendo resultado final grayscale para BGR")
        nova_img = cv2.cvtColor(nova_img, cv2.COLOR_GRAY2BGR)
    elif nova_img.shape[2] != 3:
        print(f"🔹 Ajustando resultado final canais de {nova_img.shape[2]} para 3")
        nova_img = cv2.resize(nova_img, tamanho)
...
def salvar_BalanceamentoDividido(num_classes):
    ...
    os.makedirs(pasta_salvar + "\\Treino\\", exist_ok=True)
    os.makedirs(pasta_salvar + "\\Validacao\\", exist_ok=True)
    os.makedirs(pasta_salvar + "\\Teste\\", exist_ok=True)
